...
public class MemberDaoImple implements IMemberDao{
	//싱글톤만들기 1
	private static MemberDaoImple daoImple;
	...
	public int updateMember2(Map<String, String> map) {
		...
		try {
			conn = DBUtill3.getConnection();
			String sql = "update mymember set "+ map.get("fieldName") + "=? where mem_id =?";
			
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, map.get("data"));
			pstmt.setString(2, map.get("mem_id"));
